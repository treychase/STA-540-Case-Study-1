# STA 540 Case-Study 1

# Study Overview

## Background/Motivation

Advertisements on platforms such as social media sites, dating apps, and search engines are common ways for promotions to reach consumers. However, it remains unclear which platform is most efficient in promoting home HIV self-testing among minority men who have sex with men (MSM), given that users of various platforms may have different characteristics that impact their readiness for HIV testing. While these platforms have been widely adopted for HIV prevention recruitment, there is a lack of head-to-head comparisons evaluating their relative effectiveness in promoting HIV self-testing and facilitating test kit orders.

## Primary Objective

To compare the relative effectiveness of three types of web-based platforms in promoting HIV self-testing among young (18-30 years old) minority (Black or Latinx) MSM at increased risk of HIV infection:
- **Social media sites** (Facebook, Instagram, Twitter)
- **Dating apps** (Grindr, Jack'D, Hornet)
- **Information search sites** (Google, Bing, Yahoo)

Test kit order rates are the primary measurement of effectiveness of each platform type.

## Secondary Objectives

1. To evaluate the association between HIV self-test kit ordering and key moderating variables, including:
   - Substance use patterns
   - Psychological readiness to test (based on the transtheoretical model/stages of change)
   - Attitudes toward HIV testing and treatment
   - HIV-related stigma
   - Medical mistrust

2. To assess differences in participant characteristics between those who ordered versus those who did not order an HIV test kit

3. To record and analyze advertisement campaign performance metrics (reach, engagement, and cost-effectiveness) across different platforms

# Table 1 Reproduced

![B1EB8CD3-4BD0-49E8-A675-2D19E5E9F27A](https://github.com/user-attachments/assets/71a59591-706c-475b-a790-ff0875b5d011)

# Secondary Results
![B5FD687F-2DFE-471F-9CEF-D8D8ED70D3EC](https://github.com/user-attachments/assets/08479a8b-59ee-485f-b47b-9b12ed6664c1)
![90380B87-47A9-4045-A322-12F290677E0D](https://github.com/user-attachments/assets/3bf46545-29f7-4d00-af1b-1a2249457c91)
![9F976CF3-107E-43C5-BABA-013C5F9EA350](https://github.com/user-attachments/assets/a0e35ea9-a988-4d74-95ec-0a34c6cca40e)
![4EB41D73-1237-4278-82B4-5982E4653C08](https://github.com/user-attachments/assets/183ce35b-b28a-4b76-8d51-1912825c48a1)
![F8CFA9FA-8666-43C9-8F85-EDB38B350DE7](https://github.com/user-attachments/assets/9ff30011-c66d-4ed6-8750-820ec6210f05)
![6C328AA3-181B-4CAA-B6A4-253F8561A277](https://github.com/user-attachments/assets/e86d2fdb-0d3d-4ebe-9b51-050d97082156)
![5C51188F-9616-4726-BA0B-23B255FD419D](https://github.com/user-attachments/assets/1e32063a-81bc-4585-95d0-e3a4ec401021)

# Reflection

Replicating Table 1 from the original study was a fairly straightforward process. Some results matched exactly, while others differed by ±5 observations, which is expected given that the provided dataset contained more observations than the original study. There was an issue identifying multiracial observations, which I addressed by implementing string detection during preprocessing to flag comma-separated or multi-digit entries (e.g., “24,25”, “24, 25”, or “2425”). The secondary analyses showed more discrepancies; for example, one row contained only two observations in Wave 1, which is inconsistent with the original data. With more time, these issues could be resolved through closer review of the data and dictionary. Despite this, many secondary findings aligned with the original results. Notably, significant differences in test kit ordering were driven by HIV-related stigma rather than substance use, medical mistrust, or risk behaviors, which were similar across groups. Overall, the main conclusions of the original study are reproducible and supported by the secondary analysis.

# References

1. Stafylis, C., Vavala, G., Wang, Q., McLeman, B., Lemley, S. M., Young, S. D., Xie, H., Matthews, A. G., Oden, N., Revoredo, L., Shmueli-Blumberg, D., Hichborn, E. G., McKelle, E., Moran, L. M., Jacobs, P., Marsch, L. A., & Klausner, J. D. (2022). Relative Effectiveness of Social Media, Dating Apps, and Information Search Sites in Promoting HIV Self-testing: Observational Cohort Study. *JMIR Formative Research*, 6(9), e35648. https://doi.org/10.2196/35648

2. ClinicalTrials.gov Identifier: NCT04155502. https://clinicaltrials.gov/ct2/show/NCT04155502

3. National Institute on Drug Abuse (NIDA) Clinical Trials Network (CTN) Protocol 0083. https://nida.nih.gov/research-topics/clinical-trials-network

4. Lenth, R. V. (n.d.). Comparisons and contrasts in emmeans. The Comprehensive R Archive Network. Retrieved from https://cran.r-project.org/web/packages/emmeans/vignettes/comparisons.html

